...
function montarContextoGeracaoLead(dadosLead, textoOriginalFinal, textoExtraidoViaVisao, siteParaRaspar, ehSiteAtualLinkSocial) {
  ...
  if (siteParaRaspar) {
    ...
    if (fotoPerfilIG) {
      contexto.imagensReais = [{ url: fotoPerfilIG, alt: dadosLead.nomeCliente }];
    } else {
      // CASO SARA SINGER: quando o lead só tem Instagram (sem site próprio)
      // e o Instagram bloqueia a leitura da foto de perfil pro nosso
      // servidor (o comum — Instagram exige navegador/login pra isso), o
      // site acaba sem NENHUMA foto real confirmada. Sem esse aviso, o
      // resto do pipeline segue e às vezes usa um ícone genérico (favicon/
      // logo do próprio Instagram) como se fosse a foto da pessoa — o que
      // só se percebe olhando o site pronto. Marcamos aqui pra o QA
      // (validarEstruturaDoPrototipo, 03_IA.gs) avisar na coluna Z
      // (Avisos_Validacao) que essa é a hora de buscar foto alternativa
      // manualmente (fluxo já usado com M&B e outros).
      contexto.origemInstagramSemFoto = true;
    }
  }
